...
export default function DaisyAccordionComponent({children,title,extra}:{children:any,title:string,extra:any}){

    return(
        <div className="relative collapse border border-border rounded-lg">
            <input type="checkbox" className="peer w-full h-full" />
            <div className={`collapse-title bg-transparent flex items-center px-2 py-2 peer-checked:hidden`}>
                <div className={'min-w-7'}><ChevronLeftIcon className={'h-7 w-7 '} /></div>
                <h4 className={'text-right'}>{title}</h4>
                {extra}
            </div>
            <div className={`collapse-title bg-transparent hidden px-2 py-2 peer-checked:flex transition-all`}>
