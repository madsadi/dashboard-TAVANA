...
export default function SymbolSearchSection(props: SymbolSearchProps) {
  ...
  const [arrayOfSymbols, setArrayOfSymbols] = useState<
    { faInsCode: string; instrumentId: string }[]
  >([]);
  ...
  const searchHandler = (item: string, page: number) => {
    ...
    if (item.length > 2) {
      if (searchItem === item) {
        setPage(page + 1);
        setSearchApi(
          `/SearchInstrumentBySymbolTitle?SymbolTitle=${item}&IsValid=true&pageNumber=${
            page + 1
          }`
        );
      } else {
        ...
        setFindings([]);
        setSearchApi(
          `/SearchInstrumentBySymbolTitle?SymbolTitle=${item}&IsValid=true&pageNumber=${page}`
        );
      }
    }
  };
  ...
  const select = (symbol: any) => {
    if (isMultiple) {
      setArrayOfSymbols((prevState) => [
        ...prevState,
        { faInsCode: symbol.faInsCode, instrumentId: symbol.instrumentId },
      ]);
      queryUpdate(title, [
        ...arrayOfSymbols.map((item) => item.instrumentId),
        symbol.instrumentId,
      ]);
    } else {
      setOpen(false);
  ...
  const removeHandler = (index: number) => {
    ...
    queryUpdate(
      title,
      _arrayOfSymbols.map((item) => item.instrumentId)
    );
  };
  return (
    <div className={`relative`} ref={wrapperRef}>
                ...
                findings.map((symbol: any) => {
                  return (
                    <li
                      ...
                      key={symbol.instrumentId}
                      onClick={() => select(symbol)}
                    >
                      <div>
                        {symbol.faInsCode} - {symbol.faInsName}
                      </div>
                    </li>
                  );
                })
      ...
      {isMultiple && Array.isArray(query?.[title]) && query?.[title].length ? (
        <div className="flex flex-wrap gap-2 mt-1">
          {arrayOfSymbols.map((item, index) => (
            <div
              key={item.faInsCode}
              className="rounded-full cursor-pointer text-sm bg-border px-2 py-1"
