...
export default function CSDIPortfoComparisonToolbar(props: { toggleAction: (keyword: string, visible: boolean) => void, data: any }) {
    ...
    const option1: any = {
        ...
        title: {
            text: data?.date?.first ? `${moment(data?.date?.first).locale('fa').format('YYYY-MM-DD')}` : '',
        ...
        series: [
            {
                ...
                tooltip: {
                    ...
                    renderer: function (params: AgPieSeriesTooltipRendererParams) {
                        return {
                            content: state === 'lastPrice' ? (params.datum.firstLastPricePercentage).toFixed(2) + '%' : (params.datum.firstClosingPricePercentage).toFixed(2) + '%'
                        };
                    },
                },
            },
    ...
    const option2: any = {
        data: data?.result?.pagedData || [],
        title: {
            text: data?.date?.second ? `${moment(data?.date?.second).locale('fa').format('YYYY-MM-DD')}` : '',
            fontFamily: "Yekan Bakh"
        },
        legend: {
            position: "right", // 'bottom', 'left', 'top'
        ...
        series: [
            {
                ...
                tooltip: {
                    ...
                    renderer: function (params: AgPieSeriesTooltipRendererParams) {
                        return {
                            content: state === 'lastPrice' ? (params.datum.secondLastPricePercentage).toFixed(2) + '%' : (params.datum.secondClosingPricePercentage).toFixed(2) + '%'
                        };
                    },
                },
            },
        ...
        <div className={'border-x border-border'}>
            <div className={'p-2'}>
                <div className="flex items-center justify-between">
                    <SwitchToggle isChecked={state === 'lastPrice'} onChange={changeHandler} labelBefore={'قیمت پایانی'} labelAfter={'قیمت آخرین  معامله'} />
