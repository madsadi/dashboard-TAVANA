...
export default function Edit() {
  ...
  const {
    ...
    service,
    modules,
  } = useSearchFilters(ModuleIdentifier.MARKET_RULES_MANAGEMENT, "extraAdd");
  const {
    selectedRows,
    fetchData,
    dynamicOptions,
    query: rulesQuery,
    setSelectedRows,
  } = useContext<any>(MarketRulesContext);
  const { fetchAsyncData: remoteUrl } = useQuery({});
  const [modal, setModal] = useState<boolean>(false);
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/UpdateRule`,
    method: "PUT",
  });
  ...
  const expressionTranslate = (expression: string) => {
    ...
    _array.map((item: any) => {
      if (dynamicOptions?.find((field: any) => field.name === item)) {
        ...
        _faArray.push(
          `"${
            dynamicOptions?.find((field: any) => field.name === item)
              ?.displayName
          }"`
        );
    ...
    if (!modal) {
      ...
      if (selectedRows[0]) {
        let _query: any = {
          ...
          sequenceNumber: selectedRows[0]?.sequenceNumber,
          errorMessage: selectedRows[0]?.errorMessage,
          isActive: selectedRows[0]?.isActive,
        };
        setQuery(_query);
      ...
      >
        <form onSubmit={submitForm}>
          <div className={"grid grid-cols-4 gap-4"}>
            ...
            <div className={"col-span-2 flex items-center"}>
                  ...
                  <>
                    ...
                    <div className="relative rounded">
                      <Listbox
                        ...
                        onChange={(e) =>
                          expressionQueryUpdate(
                            "value",
                            valueOptions.find((item: any) => item.code === e)
                              ?.title
                          )
                        }
                      >
                        {({ open }) => (
                          <div className="relative">
                            <Listbox.Button className="relative flex min-w-full cursor-pointer rounded-r-md border border-border bg-white py-1.5 px-2 shadow-sm focus:border-border focus:outline-none">
                            ...
                            >
                              <Listbox.Options className="absolute z-10 mt-1 min-w-full max-h-56 divide-y divide-border bg-white border border-border overflow-auto custom-scrollbar rounded-md focus:outline-none">
                                {!expressionQuery?.variable?.isRemote && (
                                  <input
                                    className={
                                      "w-full p-1.5 border border-border rounded-md"
                                    }
                                    type={
                                      expressionQuery?.variable?.valueType ===
                                      "int"
                                        ? "number"
                                        : "text"
                                    }
              ...
              <Button
                label={"اضافه"}
                className={`rounded-l ${
                  expressionQuery?.value ? "" : "bg-border text-gray-300"
                } h-[34px] mt-auto border-r-0 px-2 border border-border`}
                onClick={(e) => {
                  e.preventDefault();
                  if (expressionQuery?.variable?.fieldType === "string") {
                    ...
                    setExpression([
                      ...
                      valueOptions.length
                        ? valueOptions.find(
                            (item: any) => item.title === expressionQuery?.value
                          ).id
                        : expressionQuery?.value,
                    ]);
                    setFaExpression([
          ...
          <div>
              ...
              {faExpression.map((item: string, index) => {
                let appearance: any = (
                  <div className={"flex items-center cursor-pointer text-sm"}>
                    {item}
                    <XCircleIcon className={"h-3 w-3 text-black"} />
                  </div>
                );
                return (
                  <Badge key={index} onClick={() => remove(index)}>
                    {appearance}
                  </Badge>
                );
              })}
            ...
            <textarea
              className={"w-full border border-border rounded shadow-sm p-2"}
              placeholder={"عبارت"}
            ...
            <div className={"text-left ltr my-2"}>
              ...
              <div className={"flex flex-wrap gap-2 my-2"}>
                {expression.map((item: string, index) => {
                  let appearance: any = (
                    <div className={"flex items-center cursor-pointer text-sm"}>
                      {item} <XCircleIcon className={"h-3 w-3 text-black"} />
                    </div>
                  );
                  return (
                    <Badge key={index} onClick={() => remove(index)}>
