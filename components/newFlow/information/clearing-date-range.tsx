...
export default function ClearingDateRange() {
    ...
    const renderFromDateInput = ({ ref }: { ref: any }) => (
        <input readOnly ref={ref}
            ...
            aria-describedby="username1-help" className="block w-full text-center grow" placeholder={"تا تاریخ"} />
    )
    const renderToDateInput = ({ ref }: { ref: any }) => (
        <input readOnly ref={ref}
            value={toDate ? `${toDate?.year}-${toDate?.month}-${toDate?.day}` : ''}
            aria-describedby="username1-help" className="block w-full text-center grow" placeholder={"از تاریخ"} />
    )

    const submitHandler = async () => {
        await mutate({
            fromDate: `${fromDate?.year}${fromDate && fromDate?.month < 10 ? `0${fromDate?.month}` : fromDate?.month}${fromDate && fromDate?.day < 10 ? `0${fromDate?.day}` : fromDate?.day}`,
