        ...
        <DaisyAccordionComponent title={"اطلاعات مالی"}>
          <div className="grid md:grid-cols-4 grid-cols-2  gap-3">
            <LabelValue
              title={"ارزش دارایی"}
              value={
                profile?.assetsValue ? formatDecimals(profile?.assetsValue) : ""
              }
            />
            <LabelValue
              title={"متوسط درآمد ماهیانه"}
              value={
                profile?.inComingAverage
                  ? formatDecimals(profile?.inComingAverage)
                  : ""
              }
            />
            <LabelValue
              title={"مبلغ معاملات بورسهای اوراق بهادار و فرابورس"}
              value={
                profile?.sExchangeTransaction
                  ? formatDecimals(profile?.sExchangeTransaction)
                  : ""
              }
            />
            <LabelValue
              title={"مبلغ معاملات بورسهای کالایی"}
              value={
                profile?.cExchangeTransaction
                  ? formatDecimals(profile?.cExchangeTransaction)
                  : ""
              }
            />
            <LabelValue
              title={"مبلغ معاملات بورسهای خارج از کشور (یورو)"}
              value={
                profile?.outExchangeTransaction
                  ? formatDecimals(profile?.outExchangeTransaction)
                  : ""
              }
            />
            <LabelValue
              ...
              value={
                profile?.legalPerson
                  ? transactionLevelLegalPersonEnums.find(
                      (item: { id: string; title: string }) =>
                        item.id === profile?.transactionLevel
                    )?.title
            ...
            />
            <LabelValue
              title={" میزان آشنایی با مفاهیم مالی"}
              value={
                tradingKnowledgeLevelEnums.find(
                  (item: { id: string; title: string }) =>
                    item.id === profile?.tradingKnowledgeLevel
                )?.title
              }
            />
            <LabelValue
              title={"هدف از سرمایه گذاری در بورس کالای ایران"}
              value={profile?.companyPurpose}
            />
            <LabelValue
              title={"نام مرجع رتبه بندی کننده"}
