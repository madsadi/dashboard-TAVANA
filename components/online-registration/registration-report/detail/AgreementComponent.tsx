...
export default function AgreementComponent() {
    ...
    const { data: agreements } = useQuery({ url: `${FILE_SERVER}/api/admin-file-manager/get-content`, params: { UserId: userId, FileOwnerSoftware: 1 }, revalidateOnMount: isAllowed({ userPermissions, whoIsAllowed: [['FileManagerSystem', 'FileMananger', 'Read'].join('.')] }) })
