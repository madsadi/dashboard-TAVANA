...
export default function AgreementComponent() {
    ...
    const { data: agreements } = useQuery({ url: `${FILE_SERVER}/api/admin-file-manager/get-content`, params: { UserId: userId, FileOwnerSoftware: 1 }, revalidateOnMount: restriction ? isAllowed({ userPermissions, whoIsAllowed: [[service?.[1], modules?.[1]?.[0], 'Read'].join('.')] }) : true })
