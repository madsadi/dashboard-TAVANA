...
export default function AgentComponent() {
  ...
  return (
    <>
      {profile ? (
        <DaisyAccordionComponent title={"وکیل / نماینده"}>
          <div className="grid md:grid-cols-4 grid-cols-2  gap-3">
            <LabelValue
              title={"مشخص کننده نوع نماینده"}
              value={
                agentTypeEnums.find((item: any) => item.id === profile?.type)
                  ?.title
              }
            />
            <LabelValue
              title={"مشخص کننده تایید نماینده"}
              value={profile?.isConfirmed ? "تایید شده" : "تایید نشده"}
            />

            <LabelValue
              title={"تاریخ انقضای نمایندگی"}
              value={
                profile?.expirationDate
                  ? jalali(profile?.expirationDate)?.date
                  : "-"
              }
            />
            ...
            <LabelValue
              ...
              value={meta?.BirthCertificateNumber}
            />
            <LabelValue
              title={"شماره سریال شناسنامه"}
              value={meta?.BirthCertificateSerial?.SerialNumber}
            />
            <LabelValue
              title={"حرف سریال شناسنامه"}
              value={meta?.BirthCertificateSerial?.SerialLetter}
            />
            <LabelValue
              title={"سری سریال شناسنامه"}
              value={meta?.BirthCertificateSerial?.SerialSeri}
            />
            <LabelValue
              title={"المثنی "}
              value={meta?.birthCertificateSerial?.IsReplica ? "بله" : "خیر"}
            />
            <LabelValue title={"آدرس"} value={meta?.Address} />
            <LabelValue
              title={"شهر محل سکونت"}
              value={meta?.BirthCertificateCity}
            />
          </div>
