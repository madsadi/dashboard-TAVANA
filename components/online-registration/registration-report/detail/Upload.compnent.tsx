...
export default function UploadComponent({
    ...
    const onChange = async (imageList: any, addUpdateIndex: any) => {
        if (isAllowed({ userPermissions, whoIsAllowed: [['FileManagerSystem', 'FileMananger', 'Upload'].join('.')] })) {
    ...
    const lockHandler = async () => {
        if (lock) {
            if (isAllowed({ userPermissions, whoIsAllowed: [['FileManagerSystem', 'FileMananger', 'LockAndUnlock'].join('.')] })) {
    ...
    return (
        <div className={'pb-5 mt-4'}>
            <div className='flex items-center text-xs mb-2'>
                {item.title}
                <button className={'mr-2 cursor-pointer tooltip disabled:cursor-not-allowed'}
                    disabled={!isAllowed({ userPermissions, whoIsAllowed: [['FileManagerSystem', 'FileMananger', 'LockAndUnlock'].join('.')] })}
                    ...
                    <div className="upload__image-wrapper max-h-[150px] h-[150px] aspect-video">
                        {imageList.length > 0 ? imageList.map((image, index) => {
                            ...
                            <button
                                ...
                                {...dragProps}
                                disabled={!isAllowed({ userPermissions, whoIsAllowed: [['FileManagerSystem', 'FileMananger', 'Upload'].join('.')] })}
                            >
