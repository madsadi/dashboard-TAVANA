...
export default function EditRequest() {
  ...
  const {
    ...
    query: searchQuery,
    selectedRows,
  } = useContext<any>(AssetSwitchContext);
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/UpdateAssetSwitch`,
    method: "PUT",
  });
  ...
  const openHandler = () => {
    if (selectedRows?.length) {
      ...
      throwToast({
        type: "warning",
        value: "برای ویرایش حداقل یک مورد انتخاب کنید",
      });
    }
  };
  ...
  return (
    <>
      <Button
        label={"ویرایش درخواست تغییر ناظر"}
      ...
      />
      <Modal
        title={"ویرایش درخواست تغییر ناظر"}
        setOpen={setModal}
        open={modal}
      >
        <div className="field mt-4">
          <form className={"grid grid-cols-2 gap-4"}>
            {toolbar
              .filter((input: any) =>
                selectedRows.length > 1 ? !input.readOnly : input
              )
              .map((item: any) => {
                return (
                  <InputComponent
