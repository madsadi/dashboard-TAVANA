...
export default function Permissions() {
  ...
  const { fetchAsyncData: servicePermissions } = useQuery({
    url: `${IDP}/api/service-permossion/get-all-service-permission`,
  });
  const { mutate: addPermissionToRole } = useMutation({
    url: `${IDP}/api/roles/add-permission-to-role`,
  });
  const { mutate: removePermissionFromRole } = useMutation({
    url: `${IDP}/api/roles/remove-permission-from-role`,
  });
  ...
  const openHandler = () => {
    if (selectedRows.length) {
      ...
      const fetchUserRoles = async () => {
        await getRolePermission({ id: selectedRows[0].id }).then((res) => {
        ...
        <div className="mt-4">
          <div className={"grid grid-cols-2 gap-3"}>
            <div>
              ...
              <div
                className={
                  "border border-gray-400 rounded-md p-3 h-[300px] overflow-y-auto space-y-2 custom-scrollbar"
                }
              >
                <div
            ...
            <div>
              ...
              <div
                className={
                  "relative border border-gray-400 rounded-md p-3 h-[300px] overflow-y-auto space-y-2 custom-scrollbar"
                }
              >
                <input
                  className={"w-full"}
