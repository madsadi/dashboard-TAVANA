...
export default function LockOut() {
  ...
  const { mutate } = useMutation({
    url: `${IDP}/api/users/set-lockout-end-date`,
  });
  ...
  const lockHandler = async (e: any) => {
    ...
    await mutate({
      ...
      lockoutEndDateTime: query.lockoutEndDateTime,
    })
      .then(() => {
  ...
  return (
    <>
      <Button
        label={"قفل/باز کردن حساب کاربری"}
        onClick={openHandler}
        allowed={
          restriction
            ? [[service?.[0], modules?.[0]?.[0], "SetLockoutEndDate"].join(".")]
            : []
        }
      />
      <Modal title={"قفل/باز کردن حساب کاربری"} setOpen={setModal} open={modal}>
