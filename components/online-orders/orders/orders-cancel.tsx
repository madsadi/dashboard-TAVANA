...
export default function OrdersCancel() {
  ...
  const { restriction, service, modules } = useSearchFilters(
    ModuleIdentifier.ONLINE_ORDERS
  );
  const { query, fetchData, selectedRows, setSelectedRows } =
    useContext<any>(OrdersContext);
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/Cancel`,
  });
        ...
        .catch((err: any) => {
          throwToast({
            ...
            value: `${
              ...
              errors.find(
                (item: any) =>
                  item.errorCode === err?.response?.data?.error?.code
              )?.errorText
            }`,
          });
        })
  ...
  return (
    <>
      <Button
        ...
        allowed={
          restriction
            ? [[service[1], modules?.[1]?.[0], "CancelUserOrder"].join(".")]
            : []
        }
      />
