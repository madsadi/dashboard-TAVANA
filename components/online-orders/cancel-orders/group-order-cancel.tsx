...
export default function GpOrderCancel() {
  ...
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/GlobalCancel/CancelAllOrderForInstrumentGroup`,
  });
  const { toolbar, modules, service, restriction } = useSearchFilters(
    ModuleIdentifier.ONLINE_CANCEL,
    "gpOrder"
  );
      ...
      .catch((err) => {
        throwToast({
          ...
          value: `${
            ...
            errors.find(
              (item: any) => item.errorCode === err?.response?.data?.error?.code
            )?.errorText
          }`,
        });
      })
  ...
  return (
    <>
      <Button
        label={"حذف سفارش گروه"}
        className="bg-error"
        onClick={() => setModal(true)}
        allowed={
          restriction
            ? [
                [service[1], modules[1][0], "GlobalCancelOrderRequest"].join(
                  "."
                ),
              ]
            : []
