...
export default function Edit() {
  ...
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/${page.api}/Update`,
    method: "PUT",
  });
  ...
  useEffect(() => {
    if (toolbar && selectedRows[0]) {
      ...
      toolbar?.map((item: any) => {
        if (
          typeof ToEdit[item.title] == "string" ||
          typeof ToEdit[item.title] == "number"
        ) {
  ...
  const editHandler = async (e: any) => {
    ...
    await mutate({
      ...
      addressId: selectedRows[0]?.address?.id,
    })
      .then((res) => {
  ...
  return (
    <>
      ...
      />
      <Modal
        title={` ویرایش ${page?.searchFilter} `}
