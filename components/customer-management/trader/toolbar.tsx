...
export const TraderToolbar = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const { mutate: edit } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/trader/Update`,
    method: "PUT",
    onSuccess: () => {
      fetchData();
  ...
  const { mutate: remove } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/trader/Delete`,
    method: "DELETE",
    onSuccess: () => {
      fetchData();
  ...
  const { mutate: add } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/trader/Add`,
    method: "POST",
    onSuccess: () => {
      fetchData();
  ...
  const { fetchData, selected } = useContext<any>(CustomerManagementTrader);
