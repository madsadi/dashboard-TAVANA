...
export default function AddNew() {
  ...
  const addNewHandler = async (e: any, query: any) => {
    ...
    Object.keys(query).map((item: any) => {
      if (typeof query[`${item}`] === "object" && query[`${item}`]) {
        let value = "";
        Object.keys(query[`${item}`])
          .sort()
          .map((child: any) => {
            if (
              query[`${item}`][`${child}`] !== undefined &&
              query[`${item}`][`${child}`] !== null &&
              query[`${item}`][`${child}`] !== ""
            ) {
  ...
  return (
    <>
      ...
      />
      <Modal
        title={` ایجاد${page?.searchFilter} جدید `}
      ...
      >
        <div className="field mt-4">
          ...
          {page?.api === "customerAgreement" && (
            <ExtraDateAndTimeInput setQuery={setQuery} query={query} />
          )}
