...
export const LegalPersonstakeholderToolbar = ({ isMainPage = false }) => {
  ...
  const { mutate: deleteHandler } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/legalPersonstakeholder/Delete`,
    method: "DELETE",
    onSuccess: () => {
      fetchData();
  ...
  const { mutate: add } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/legalPersonstakeholder/Add`,
    method: "POST",
    onSuccess: () => {
      fetchData();
  ...
  const { fetchData, selected, customer } = useContext<any>(
    CustomerManagementLegalPersonStakeholderContext
  );
  ...
  return (
    ...
    >
      <CRUDWrapper
        ref={addRef}
        title={`ذینفعان مشتری حقوقی`}
        confirmHandler={(e, query) => {
          e.preventDefault();
          add({ customerId: customer.customerId, ...query });
        }}
        loading={loading}
        module={ModuleIdentifier.CUSTOMER_MANAGEMENT_legal_person_stakeholders}
        subModule="modal"
        mode="edit"
      ...
      <CRUDWrapper
        ref={editRef}
        title={`ویرایش ذینفعان مشتری حقوقی`}
        mode="edit"
        confirmHandler={(e, query) => {
          e.preventDefault();
      ...
      <CRUDWrapper
        ...
        module={ModuleIdentifier.CUSTOMER_MANAGEMENT_legal_person_stakeholders}
        selectedItem={selected}
        modalMessage="حذف ذینفعان مشتری حقوقی"
