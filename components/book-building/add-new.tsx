...
export default function AddModal() {
  ...
  const { mutate } = useMutation({
    url: `${ADMIN_GATEWAY}/api/request/addBookBuilding`,
  });
    ...
    if (query?.InstrumentId && query?.maxQuantity) {
      ...
      await mutate({
        ...
        fromActiveDateTime:
          ...
          `${query?.startMinute ? query?.startMinute + ":00.000Z" : ""}`,
        toActiveDateTime:
          moment(query.EndDate).locale("en").format("YYYY-MM-DD") +
          `${query?.endHour ? "T" + query?.endHour + ":" : ""}` +
          `${query?.endMinute ? query?.endMinute + ":00.000Z" : ""}`,
      })
        .then(() => {
          setModal(false);
      ...
      if (!query.maxQuantity) {
        throwToast({
          type: "warning",
          value: "بیشینه حجم سفارش را لطفا وارد کنید",
        });
