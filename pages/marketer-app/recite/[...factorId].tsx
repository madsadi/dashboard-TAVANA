...
function FactorIdEdit() {
    ...
    useEffect(() => {
        if (data?.result.pagedData?.length) {
            ...
            {
                toolbar.map((item: any) => _initialValue[item.title] = data.result.pagedData[0][item.title])
            }
    ...
    return (
        <div className={'flex flex-col h-full flex-1'}>
            ...
            <Button label="ثبت ویرایش" className={'w-fit mr-auto bg-secondary'} onClick={openHandler} />
        </div>
    )
}

export default withPermission(FactorIdEdit, ModuleIdentifier.MARKETER_APP_recite)